...
2. **Assistente opcional** (`scripts/install-wizard.sh`): em terminal interativo, **ENTER** aceita a configuração recomendada (cópia local do repositório para `/home/administrador/raspberry-pi-manager`); em seguida pergunta se quer **troca de IPv4 temporária para o `apt`** (útil quando `deb.debian.org` devolve **403** na rede atual). Se responder **s**, o IPv4 temporário (**10.0.8.94/16**, gateway **10.0.0.1** por omissão) é aplicado **de seguida**, antes da limpeza e dos passos **[1.5]**–**[2]**. **`o`** abre opções completas (GitHub, pasta, rede, limpeza alargada). Em automação: `PI_MANAGER_INSTALL_INTERACTIVE=0`. Variáveis `PI_*` antes do `sudo`: **`sudo -E ./install.sh`**.
...
5. **Rede opcional para atualizações**: se `PI_MANAGER_NETWORK_SWAP_FOR_UPDATE=1`, o script guarda o IPv4 atual, aplica **10.0.8.94/16** com gateway **10.0.0.1** **logo após o assistente** (ou no início do `install.sh` se a variável já estiver definida), mantém durante o `apt`, e restaura no fim — requer `nmcli` / NetworkManager (ver tabela mais abaixo).
...
**Atualização com IPv4 temporário (Raspberry Pi / NetworkManager):** variáveis de **ambiente** (não vão em `/etc/default` por omissão; use `export` no cron, webhook ou `sudo env …`). Com **`PI_MANAGER_NETWORK_SWAP_FOR_UPDATE=1`**, o `install.sh` **aplica o swap logo após o assistente** (antes de ping, pré-voo e `apt upgrade`) e o `update_app.sh` aplica antes do seu `apt`; ambos **guardam** a ligação IPv4 atual, aplicam um endereço **estático temporário** e **restauram** no fim ou em caso de erro (via `trap`). **Preferência:** `nmcli` + NetworkManager ativo; **se não existir**, o script usa **`iproute2` (`ip`)** para mudar de range **antes** de qualquer `apt install network-manager`, para o `apt` já usar o IP correto. O pacote `network-manager` continua a ser instalado no passo **[3/12]** (dependências), quando aplicável.
...
| `PI_MANAGER_UPDATE_PREFIX` | Prefixo CIDR (por omissão **`16`**). |
...
**Aviso:** mudar o IP pode **cortar SSH** se a sessão não estiver na mesma subnet; prefira consola, IPMI ou rede de gestão. Por omissão o projeto usa **10.0.8.94/16** e gateway **10.0.0.1**; ajuste **`PI_MANAGER_UPDATE_*`** se a sua rede de gestão for diferente.
...
- **`scripts/lib-network-swap-for-update.sh`**: troca temporária de IPv4 (**NetworkManager** ou **`iproute2`** se não houver `nmcli`) durante `install.sh` / `update_app.sh` quando **`PI_MANAGER_NETWORK_SWAP_FOR_UPDATE=1`** (ver tabela acima).
...
**Solução prevista no projeto:** no assistente, após escolher a configuração recomendada (ENTER), responda **s** à pergunta **«Ativar troca de IPv4 temporária para o apt?»** — o script aplica **10.0.8.94/16** com gateway **10.0.0.1** (predefinições do projeto) **de imediato** (via NetworkManager se existir; caso contrário via **`ip`**), sem precisar de `apt install network-manager` antes; depois corre **[1.5]**, **[1.6]**, **[2/12]** e **[3/12]** com o **mesmo** IP temporário (incl. pacotes de `deb.debian.org` como `python3-pam`), e **só então repõe** a rede / NetworkManager. Em falha de `apt`, o script **tenta repor** a configuração anterior (incl. `trap` no `EXIT`). Se a troca falhar por falta de `iproute2` ou rede, instale dependências ou ajuste **`PI_MANAGER_UPDATE_*`** conforme a sua rede.

**Variáveis por linha de comandos** (útil em scripts): `PI_MANAGER_NETWORK_SWAP_FOR_UPDATE=1` basta — omissões **10.0.8.94/16** e gateway **10.0.0.1**; ajuste **`PI_MANAGER_UPDATE_PREFIX`** se necessário. Corra **`sudo -E ./install.sh`** — o **`sudo` sem `-E` apaga as variáveis** `PI_*` do seu utilizador.
